...
class PoseFGSTrain(object):
    """
    PoseFGS train model for prediction
    """
    def __init__(self, args):
        ...
        """ modules """
        self.DATASET = import_module("src."+self.dataset)
        self.MODEL = import_module("src."+self.model)
        ...
        """ local directory """
        file_folder = "Train_dataset_{}_model_{}_info_{}_{}_cuda{}"
        ...
        self.model_path = self.path + "models/" + file_folder
    ...
    def train_one_fold(self):
        ...
        # pickle.dump(self.Dataset.table.loc[self.index_train]['pdbid'], open("train_{}_{}.pkl".format(self.repeat, self.fold), "wb"))
        # pickle.dump(self.Dataset.table.loc[self.index_valid]['pdbid'], open("valid_{}_{}.pkl".format(self.repeat, self.fold), "wb"))
    ...
    def train_one_epoch(self):
        ...
        self.Model.train()
        self.Dataset.reset_index(self.index_train, True)
    ...
    def evaluate(self, dataset):
        ...
        if dataset == "Train":
            self.Dataset.reset_index(self.index_train, False)
        elif dataset == "Valid":
            self.Dataset.reset_index(self.index_valid, False)
    ...
    def save(self):
        torch.save(self.Model, self.model_path + "best_model_{}_{}.pt".format(self.repeat, self.fold))
        torch.save(self.Model.state_dict(), self.model_path + "best_model_{}_{}.pt".format(self.repeat, self.fold))
        logging.info("Best model saved at " + self.model_path)
...
def main():
    ...
    # define task
    parser.add_argument("--dataset", default='general2016', help="dataset", type=str)
    ...
    posefgs = PoseFGSTrain(args)
        
    """ Train """
    posefgs.train_and_save()
